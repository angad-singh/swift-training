...
func levelUpAttack(baseAttack: AttackTuple) -> AttackTuple {
    let increasedAttack: AttackTuple = (baseAttack.name, baseAttack.damage * 2, !baseAttack.rechargeable)
    return increasedAttack
}

var poweredAttack: AttackTuple = levelUpAttack(baseAttack: sunStrike)


// Type alias as a function parameter
/// this is a type alias for a closure
typealias ArrayClosure = (Array<String>) -> Void
...
activeMembers { (members) in
    for name in members {
        print("The length of \(name) is \(name.count)")
    }
}
